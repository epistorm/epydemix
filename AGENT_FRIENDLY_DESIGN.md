...
The key design challenge is the simulation function. The ABC sampler expects a callable that takes a parameter dictionary and returns `{"data": array}`. In interactive Python, the user writes this wrapper by hand. For the CLI, we auto-generate it from the configuration: the wrapper builds the model once, then on each call updates the calibrated parameters, runs a single simulation, and extracts the target variable. The target can be either a compartment column (for prevalence data) or a transition column (for incidence data — the more common real-world case, since observed data is typically reported as new cases, hospitalizations, or deaths per time step). This is invisible to the agent — it just specifies which variable to fit against.  
...
A deliberate omission is calibration of initial conditions. The underlying `ABCSampler` is generic enough to handle it — it samples from priors and passes a flat parameter dict to the simulation function — and in the Python API a user can write a custom `sim_fn` that parameterizes initial conditions however they like. But the config-driven surface does not expose this, because the ABC sampler treats every prior as an independent knob. Compartment initial conditions are inherently constrained: their fractions must sum to 1. The sampler has no constraint mechanism, so it would propose combinations where fractions exceed 1, waste particles on physically impossible states, and never learn why those proposals were rejected. Even the single-compartment case (calibrate `I₀`, hold everything else fixed) silently breaks population conservation. We chose to document this as a known limitation and point users who need IC sensitivity analysis to the Python API, where they can write a simulation wrapper that parameterizes the seeding size as a scalar and reconstructs the full IC dict with proper normalization on every call.
